<template>
  <div>
    <div
      v-for="maint in this.$store.state.activeMaintenance"
...
export default {
  ...
  created() {
    MaintenanceService.getById(this.$route.params.id)
      .then((response) => {
    ...
    MaintenanceService.getAddressForMaintenance(this.$route.params.id)
      .then((response) => {
